...
<div class="wrap" id="libview" hidden data-stream="photos">
	...
	<?php
	/*
	 * The public photo links. Two postures, and the panel says which is which
	 * in words rather than leaving a volunteer to infer it from a mode column:
	...
	<div class="card pad" id="ldoors">
		...
		<div id="ldoorlist" class="muted">Loading&hellip;</div>
		<details style="margin-top:12px">
			<summary style="cursor:pointer">Make a party link</summary>
			<p class="muted" style="margin:8px 0">A party link <strong>skips approval</strong>: photos go straight into the library and onto the screen. It only works between the times you set. Volunteers remove anything unwanted afterwards.</p>
			<div class="lfrow">
				<label class="lf"><span>What is it called?</span><input type="text" id="dlabel" maxlength="80" placeholder="Oktoberfest 2026"></label>
				<label class="lf"><span>Event tag</span><input type="text" id="devent" maxlength="120" placeholder="Oktoberfest 2026"></label>
				<label class="lf"><span>Place</span><select id="dplace"><option value="">&mdash; none &mdash;</option></select></label>
			</div>
			<div class="lfrow" style="margin-top:8px">
				<label class="lf"><span>Opens</span><input type="datetime-local" id="dstarts"></label>
				<label class="lf"><span>Closes</span><input type="datetime-local" id="dends"></label>
			</div>
			<div class="actions" style="margin-top:10px">
				<button class="btn" id="dgo" type="button">Create the link</button>
...
(function(){
	...
	/* ===================== public photo links ===================== */
	(function(){
		var list = document.getElementById('ldoorlist');
		if (!list) { return; }

		function draw(doors){
			if (!doors.length) { list.textContent = 'No links yet.'; return; }
			list.innerHTML = doors.map(function(d){
				var when = d.permanent
					? 'Always open — a volunteer approves each photo before it appears.'
					: (d.starts.replace('T', ' ') + ' to ' + d.ends.replace('T', ' ') +
						' — photos go straight in, no approval.');
				return '<div class="card pad" style="margin:8px 0">' +
					'<strong>' + esc(d.label) + '</strong>' +
					(d.closed ? ' <span class="muted">(' + esc(d.closed) + ')</span>'
						: ' <span class="ok">(open now)</span>') +
					'<div class="muted" style="margin:2px 0 6px">' + esc(when) + '</div>' +
					'<code class="durl" style="word-break:break-all">' + esc(d.url) + '</code>' +
					'<div class="actions" style="margin-top:8px">' +
					'<button class="btn sec dcopy" data-url="' + esc(d.url) + '" type="button">Copy link</button>' +
					'<button class="btn sec dtoggle" data-t="' + d.token + '" data-a="' + (d.active ? '0' : '1') + '" type="button">' +
						(d.active ? 'Switch off' : 'Switch on') + '</button>' +
					(d.permanent ? '<button class="btn sec dcycle" data-t="' + d.token + '" type="button">New address</button>' : '') +
					'<span class="muted">' + d.count + ' photo(s) so far</span>' +
					'</div></div>';
			}).join('');
		}

		function load(){
			api('/photos/doors').then(function(r){ draw(r.doors || []); })
				.catch(function(e){ list.textContent = e.message; });
		}

		list.addEventListener('click', function(ev){
			var c = ev.target.closest('.dcopy');
			if (c) {
				navigator.clipboard.writeText(c.dataset.url).then(function(){
					c.textContent = 'Copied';
					setTimeout(function(){ c.textContent = 'Copy link'; }, 1500);
				});
				return;
			}
			var tg = ev.target.closest('.dtoggle');
			if (tg) {
				api('/photos/doors/toggle', { method: 'POST', body: JSON.stringify({
					token: tg.dataset.t, active: tg.dataset.a === '1'
				}) }).then(load).catch(function(e){ list.textContent = e.message; });
				return;
			}
			var cy = ev.target.closest('.dcycle');
			if (cy) {
				if (!confirm('Give this link a new address? Every printed QR code and poster with the old one stops working.')) { return; }
				api('/photos/doors/cycle', { method: 'POST', body: JSON.stringify({ token: cy.dataset.t }) })
					.then(load).catch(function(e){ list.textContent = e.message; });
			}
		});
		...
		document.getElementById('dgo').onclick = function(){
			...
			api('/photos/doors', { method: 'POST', body: JSON.stringify({
				label:  document.getElementById('dlabel').value.trim(),
				event:  document.getElementById('devent').value.trim(),
				place:  document.getElementById('dplace').value,
				starts: document.getElementById('dstarts').value,
				ends:   document.getElementById('dends').value
			}) }).then(function(){
				document.getElementById('dgo').disabled = false;
		...
		var sel = document.getElementById('dplace');
		PLACES.forEach(function(pl){
			var o = document.createElement('option');
			o.value = pl.name; o.textContent = pl.label || pl.name;
			sel.appendChild(o);
		});
